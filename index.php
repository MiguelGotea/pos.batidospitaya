...
<head>
    ...
    <style>
        body { overflow: auto; }
        .pos-wrapper { min-height: 100vh; display: block; padding: 20px; }
        .pos-dashboard { max-width: 1100px; margin: 0 auto; }
        .pos-header { display: flex; justify-content: space-between; align-items: center;
                      background: var(--surface); border: 1px solid var(--border);
                      border-radius: 18px; padding: 14px 22px; gap: 16px; margin-bottom: 24px; }
        .pos-user-info { display: flex; align-items: center; gap: 14px; }
        .pos-avatar { width: 44px; height: 44px; border-radius: 50%;
        ...
        .pos-btn-exit { padding: 8px 14px; border-radius: 9px; font-size: .8rem; font-weight: 600;
                        font-family: inherit; cursor: pointer; border: 1px solid rgba(224,85,85,.35);
                        background: rgba(224,85,85,.1); color: #e05555; transition: .2s;
                        text-decoration: none; white-space: nowrap; }
        .pos-btn-exit:hover { background: #e05555; color: #fff; }
        .pos-btn-exit.secondary { border-color: var(--border); background: var(--surface); color: var(--text-muted); }
        .pos-btn-exit.secondary:hover { background: var(--surface-hover); color: #fff; }
        .pos-welcome { background: var(--surface); border: 1px solid var(--border); border-radius: 20px;
                       padding: 60px 40px; text-align: center; }
        .pos-welcome h2 { font-size: 1.6rem; font-weight: 800; margin-bottom: 12px; }
        .pos-welcome p { color: var(--text-muted); max-width: 480px; margin: 0 auto 32px; }
        .pos-actions { display: flex; justify-content: center; gap: 16px; flex-wrap: wrap; }
        .pos-action-btn { padding: 14px 28px; border-radius: 13px; border: 1px solid var(--border);
                          background: var(--surface); color: var(--text); font-family: inherit;
                          font-size: .95rem; font-weight: 600; cursor: pointer; transition: .2s; text-decoration: none; }
        .pos-action-btn:hover { background: var(--surface-hover); border-color: #51B8AC; }
